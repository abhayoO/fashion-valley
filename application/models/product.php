<?php
	class Product extends CI_Model {
		...
		public function get_all_products_by_group_subcat_id($ids) {
			$query = "SELECT DISTINCT(products.id),products.name,products.description,products.unit_price FROM products
			JOIN product_subcategories ON product_subcategories.product_id = products.id 
			JOIN subcategories ON subcategories.id = product_subcategories.subcategory_id
			WHERE subcategory_id IN (".implode(",",$ids).")";
